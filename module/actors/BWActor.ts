...
> {
        ...
        return this.forks
            .filter(
                (s) =>
                    ...
                    s.system.exp >
                        ((this.system as unknown as BWCharacterData | NpcData)
                            .ptgs.woundDice || 0)
            )
            .map((s) => {
    ...
    getRollModifiers(rollName: string): RollModifier[] {
        return (this.rollModifiers[rollName.toLowerCase()] || []).concat(
            this.rollModifiers.all || []
        );
    }
            ...
            this.items.forEach((item) => {
                ...
                switch (type) {
                    case 'skill':
                        ...
                        if (
                            (i as SkillData).skilltype === 'martial' &&
                            !(i as SkillData).training
                        ) {
                    ...
                    case 'reputation':
                        ...
                        if (rep.infamous) {
                            this.circlesMalus.push({
                                name: name,
                                amount: rep.dice,
                            });
                        } else {
                            this.circlesBonus.push({
                                name: name,
                                amount: rep.dice,
                            });
                        }
                    ...
                    case 'affiliation':
                        this.circlesBonus.push({
                            name: name,
                            amount: (i as AffiliationData).dice,
                        });
                        break;
                    case 'trait':
                                ...
                                t.dieModifierTarget
                                    ...
                                    .forEach((target) =>
                                        this._addRollModifier(
                                            target.trim(),
                                            Trait.asRollDieModifier(name, t)
                                        )
                                    );
                            }
                    ...
                    case 'melee weapon':
                    case 'ranged weapon':
                        this.fightWeapons.push(
                            item as MeleeWeapon | RangedWeapon | Spell
                        );
                        break;
                }
            });
    ...
    async _preCreate(
        actor: Partial<T> & TypeMissing,
        _options: FoundryDocument.CreateOptions,
        user: User
    ): Promise<void> {
    ...
    */
    ...
    private _calculateClumsyWeight() {
        ...
        this.items
            .filter(
                (i: BWItem) =>
                    i.type === 'armor' && (i as Armor).system.equipped
            )
            .forEach((i: Armor) => {
                const a = i.system;
                if (a.hasHelm) {
                    clumsyWeight.helmetObPenalty =
                        a.perceptionObservationPenalty || 0;
                }
                if (a.hasTorso) {
                    ...
                    clumsyWeight.swimmingPenalty = Math.max(
                        clumsyWeight.swimmingPenalty,
                        a.swimmingPenalty || 0
                    );
                }
        ...
        const baseModifier = {
            optional: true,
            label: game.i18n.localize('BW.armor.armorClumsyWeight'),
        };
        this._addRollModifier(
            'climbing',
        ...
        this._addRollModifier(
            'all',
            {
                ...
                label: game.i18n.localize('BW.armor.untrainedArmorPenalty'),
                optional: true,
            },
            true
        );
        ...
        this._addRollModifier(
            'health',
            {
                ...
                label: game.i18n.localize('BW.armor.untrainedArmor'),
                optional: true,
            },
            true
        );
