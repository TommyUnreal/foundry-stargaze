...
export class CharacterBurnerDialog extends Application {
            ...
            buttons: {
                select: {
                    ...
                    callback: async (html: JQuery) => {
                        const usedSources = html
                            .find("input[name='compendiumList']:checked")
                        ...
                        const loadingDialog = new Dialog(
                            {
                                title: '',
                                content:
                                    "<div class='loading-title'>Loading Character Burner data...</div><div class='burner-loading-spinner'><i class='fas fa-dharmachakra'></i></div>",
                                buttons: {},
                                default: '',
                            },
                        ...
                        dialog._skills = await getItemsOfType<Skill>(
                            'skill',
                            usedSources
                        );
                        dialog._traits = await getItemsOfType<Trait>(
                            'trait',
                            usedSources
                        );
                        dialog._property = await getItemsOfType<Property>(
                            'property',
                            usedSources
                        );
                        dialog._gear = await getItemsOfTypes(
                            [
                                'melee weapon',
                                'ranged weapon',
                                'armor',
    ...
    getData(): CharacterBurnerData {
        ...
        const blankLifepath: LifepathEntry = {
            name: '',
            time: 0,
            lead: 0,
            resources: 0,
        ...
        this._traits.forEach((s) => {
            const trait = {
                name: s.name,
                label: s.system.pointCost
                    ? `${s.name} - ${s.system.pointCost} Pts`
                    : s.name,
            };
        ...
        this._gear.forEach(
            (
                g: BWItem<
                    PropertyData | MeleeWeaponData | ArmorData | PossessionData
                >
            ) => {
                const entry = {
                    name: g.name,
                    label: g.system.pointCost
                        ? `${g.name} - ${g.system.pointCost} Pts`
                        : g.name,
                };
                switch (g.type) {
                    case 'melee weapon':
    ...
    activateListeners(html: JQuery): void {
        this._burnerListeners = [
            ...
            $(html.find("select[name='traitName']")).select2(
                defaultSelectOptions
            ),
            $(html.find("select[name='propertyName']")).select2(
                defaultSelectOptions
            ),
            $(html.find("select[name='itemName']")).select2(
                defaultSelectOptions
            ),
            ...
            html
                ...
                .on('change', (_) => this._storeSum(html, 'timeTotal', 'time')),
            html
                .find("input[name='lead']")
                .on('change', (_) => this._storeSum(html, 'leadTotal', 'lead')),
            html
                .find("input[name='timeTotal'], input[name='leadTotal']")
                .on('change', (_) =>
                    this._storeSum(html, 'ageTotal', 'timeTotal', 'leadTotal')
                ),

            html
                .find("input[name='resources']")
                .on('change', (_) =>
                    this._storeSum(html, 'resourcesTotal', 'resources')
                ),

            html
                .find("input[name='mentalStat']")
                .on('change', (_) =>
                    this._storeSum(html, 'mentalStatTotal', 'mentalStat')
                ),
            html
                .find(
                    "input[name='mentalStatTotal'], select[name='mentalPool']"
                )
                .on('change', (_) =>
                    this._storeSum(
                        html,
            ...
            html
                .find("input[name='physicalStat']")
                .on('change', (_) =>
                    this._storeSum(html, 'physicalStatTotal', 'physicalStat')
                ),
            html
                .find(
                    "select[name='physicalPool'], input[name='physicalStatTotal']"
                )
                .on('change', (_) =>
                    this._storeSum(
                        html,
            ...
            html
                .find(
                    "input[name='physicalPoolTotal'], input[name='physicalPoolSpent']"
                )
                .on('change', (_) =>
                    this._storeDiff(
                        html,
            ...
            html
                .find("input[name='skillPts']")
                .on('change', (_) =>
                    this._storeSum(html, 'skillPtsTotal', 'skillPts')
                ),
            html
                .find("input[name='generalSkillPts']")
                .on('change', (_) =>
                    this._storeSum(
                        html,
                        'generalSkillPtsTotal',
                        'generalSkillPts'
                    )
                ),
            html
                .find(
                    "input[name='skillPtsTotal'], input[name='generalSkillPtsTotal']"
                )
                .on('change', (_) =>
                    this._storeSum(
                        html,
            ...
            html
                .find("input[name='traitPts']")
                .on('change', (_) =>
                    this._storeSum(html, 'traitPtsTotal', 'traitPts')
                ),

            html
                .find(
                    "input[name='mentalPoolTotal'], input[name='mentalPoolSpent']"
                )
                .on('change', (_) =>
                    this._storeDiff(
                        html,
            ...
            html
                .find(
                    "input[name='willSpent'], select[name='willShadeSpent'], input[name='perceptionSpent'], select[name='perceptionShadeSpent']"
                )
                .on('change', (_) => {
                    this._storeSum(
                        html,
                        ...
                        'willShadeSpent',
                        'perceptionSpent',
                        'perceptionShadeSpent'
                    );
                }),
            html
                .find(
                    "input[name='powerSpent'], select[name='powerShadeSpent'], input[name='forteSpent'], select[name='forteShadeSpent'], " +
                        "input[name='agilitySpent'], select[name='agilityShadeSpent'], input[name='speedSpent'], select[name='speedShadeSpent']"
                )
                .on('change', (_) => {
                    this._storeSum(
                        html,
            ...
            html
                .find(
                    "input[name='skillPtsSpent'], input[name='combinedSkillPts']"
                )
                .on('change', (_) =>
                    this._storeDiff(
                        html,
            ...
            html
                ...
                .on('change', (e) => this._tryLoadSkill(e)),
            html
                .find(
                    "input[name='skillAdvances'], input[name='skillOpened'], input[name='skillTraining'], input[name='skillMagic'], select[name='skillShade']"
                )
                .on('change', (e: JQuery.ChangeEvent) =>
                    this._calculateSkillWorth(e)
                ),
            html
                .find("input[name='skillPtsWorth']")
                .on('change', (_) =>
                    this._storeSum(html, 'skillPtsSpent', 'skillPtsWorth')
                ),

            html
                .find(
                    "*[name='powerSpent'], *[name='powerShadeSpent'], *[name='forteSpent'], *[name='forteShadeSpent'], " +
                        ...
                        "*[name='custom1Spent'], *[name='custom1Shade'], *[name='custom2Spent'], *[name='custom2Shade']"
                )
                .on('change', (e) => this._calculateAllSkillExponents(e, html)),
            html
                .find(
                    "input[name='skillOpened'], input[name='skillAdvances'], select[name='skillRoot1'], select[name='skillRoot2']"
                )
                .on('change', (e: JQuery.ChangeEvent) =>
                    this._calculateSkillExponent(e.currentTarget, html)
                ),

            // trait points spent
            html
                .find("select[name='traitName']")
                .on('change', (e) => this._tryLoadTrait(e)),
            html
                .find("input[name='traitCost'], input[name='traitTaken']")
                .on('change', (_) => this._calculateSpentTraits(html)),
            html
                .find(
                    "input[name='traitPtsSpent'], input[name='traitPtsTotal']"
                )
                .on('change', (_) =>
                    this._storeDiff(
                        html,
            ...
            html
                .find(
                    "input[name='resourcePointsSpent'], input[name='resourcesTotal']"
                )
                .on('change', (_) =>
                    this._storeDiff(
                        html,
            ...
            html
                .find(
                    "input[name='reputationSpent'], input[name='relationshipsSpent'], input[name='propertySpent'], input[name='gearSpent']"
                )
                .on('change', (_) =>
                    this._storeSum(
                        html,
            ...
            html
                .find(
                    "input[name='reputationName'], input[name='reputationType'], input[name='reputationDice']"
                )
                .on('change', (e) =>
                    this._calculateRepAffCost($(e.currentTarget))
                ),
            html
                .find("input[name='reputationCost']")
                .on('change', (_e) =>
                    this._storeSum(html, 'reputationSpent', 'reputationCost')
                ),

            // relationship totals
            html
                .find(
                    "input[name='relationshipName'], select[name='relPow'], select[name='relFam'], input[name='relRom'], input[name='relFor'], input[name='relHat']"
                )
                .on('change', (e) =>
                    this._calculateRelationshipCost($(e.currentTarget))
                ),
            html
                .find("input[name='relationshipCost']")
                .on('change', (_e) =>
                    this._storeSum(
                        html,
                        'relationshipsSpent',
            ...
            // property
            html
                .find("select[name='propertyName']")
                .on('change', (e: JQuery.TriggeredEvent) =>
                    this._tryLoadProperty(e)
                ),
            html
                .find("input[name='propertyCost']")
                .on('change', (_) =>
                    this._storeSum(html, 'propertySpent', 'propertyCost')
                ),
            // gear
            html
                .find("select[name='itemName']")
                .on('change', (e: JQuery.TriggeredEvent) =>
                    this._tryLoadGear(e)
                ),
            html
                .find("input[name='itemCost']")
                .on('change', (_) =>
                    this._storeSum(html, 'gearSpent', 'itemCost')
                ),

            // extra rules totals
            html
                .find(
                    "input[name='reputationSpent'], input[name='propertySpent']"
                )
                .on('change', (_) =>
                    this._storeSum(
                        html,
                        'resourceExponentAmount',
                        'reputationSpent',
                        'propertySpent'
                    )
                ),
            html
                .find(
                    "input[name='relationshipsSpent'], input[name='propertySpent']"
                )
                .on('change', (_) =>
                    this._storeSum(
                        html,
                        'circlesExponentBonus',
                        'relationshipsSpent',
                        'propertySpent'
                    )
                ),
            ...
            html
                .find('button.submit-burner-button')
                .on('click', (e) => this._finishBurning(e, html)),

            html
                .find('.skills-grid .fa-check-circle')
                .on('click', (e) =>
                    this._openItemSheet(
                        e,
                        '.skills-grid',
            ...
            html
                .find('.burner-traits-grid .fa-check-circle')
                .on('click', (e) =>
                    this._openItemSheet(
                        e,
                        '.burner-traits-grid',
            ...
            html
                .find('.burner-property .fa-check-circle')
                .on('click', (e) =>
                    this._openItemSheet(
                        e,
                        '.burner-property',
            ...
            html
                .find('.burner-gear .fa-check-circle')
                .on('click', (e) =>
                    this._openItemSheet(
                        e,
                        '.burner-gear',
    ...
    private _openItemSheet(
        e: JQuery.ClickEvent,
        parentSelector: string,
        idSelector: string,
        items: BWItem[]
    ): void {
        const idElement = $(e.currentTarget)
        ...
        if (data.type === 'Item') {
            ...
            if (data.uuid) {
                ...
                if (data.pack) {
                    ...
                    ).getDocument(data.actorId)) as Actor;
    ...
    private _addLifepath(lp: Lifepath) {
        ...
        const emptyLifepath = $(this.element)
            .find('.lifepath-grid > input[name="lifepathName"]')
            .filter((_, e) => !$(e).val())
            .first();

        const skillList = pathData.skillList
            .split(',')
            .map((i) => i.trim())
            .filter((i) => i);
        const traitList = pathData.traitList
            .split(',')
            .map((i) => i.trim())
            .filter((i) => i);
        ...
        switch (numDuplicates) {
            ...
            case 1:
                if (traitList.length <= 1) {
                    pathData.traitPoints = Math.max(
                        pathData.traitPoints - 1,
                        0
                    );
                }
                break;
        ...
        emptyLifepath
            ...
            .val(0)
            .next()
            .val(pathData.resources)
            .trigger('change')
            .nextAll('input[name="skillPts"]')
            .first()
            .val(pathData.skillPoints)
            .trigger('change')
            .next()
            .val(pathData.generalPoints)
            .trigger('change')
            .next()
            .val(pathData.traitPoints)
            .trigger('change');

        const mentalPoints = emptyLifepath
            .nextAll('.inline-text')
            .first()
            .children('input[name="mentalStat"]')
            .first();
        const physicalPoints = emptyLifepath
            .nextAll('.inline-text')
            .first()
            .next()
            .children('input[name="physicalStat"]')
            .first();
        ...
        switch (pathData.statBoost) {
            ...
            case 'either':
                new Dialog({
                    ...
                    content:
                        '<p>This lifepath allows a stat point to be assigned to either the mental or physical pools</p>',
                    buttons: {
                        mental: {
        ...
        skillList.forEach((s, i) =>
            this._addEntry(s, 'skill', requiredIndex === i)
        );
        traitList.forEach((t, i) =>
            this._addEntry(t, 'trait', requiredIndex === i)
        );
    }
        ...
        switch (itemType) {
            case 'skill':
                element = $(this.element)
                    .find('.skills-grid > select[name="skillName"]')
                    .filter((_, e) => !$(e).val())
                    .first();
                ...
                if (
                    ...
                    if (
                        element.find(
                            `option[value='${escapeQuotes(itemName)}']`
                        ).length
                    ) {
                    ...
                    if (required) {
                        element = $(this.element)
                            .find(`select[name="skillName"]`)
                            .filter(
                                (_, element) => $(element).val() === itemName
                            )
                            .first()
                            .nextAll("input[type='checkbox']")
                            .first()
            ...
            case 'property':
                ...
                // Set the value, creating a new option if necessary
                if (
                    element.find(`option[value='${escapeQuotes(itemName)}']`)
                        .length
                ) {
                    element.val(itemName).trigger('change');
                } else {
            ...
            case 'affiliation':
                element = $(this.element)
                    .find('input[name="reputationName"]')
                    .filter((_, e) => !$(e).val())
                    .first();
                element.val(itemName).trigger('change');
                element
                    .nextAll('input[type="checkbox"]')
                    .first()
                    .prop('checked', itemName === 'reputation')
                    .trigger('change');
                element
                    .nextAll('input[type="number"]')
                    .first()
                    .val((itemData as ReputationData | AffiliationData).dice)
                    .trigger('change');
                break;
            case 'relationship':
                const rel = itemData as RelationshipData;
                element = $(this.element)
                    .find('input[name="relationshipName"]')
                    .filter((_, e) => !$(e).val())
                    .first();
                ...
                if (rel.romantic) {
                    element
                        .nextAll('input[name="relRom"]')
                        .prop('checked', true);
                }
                if (rel.forbidden) {
                    element
                        .nextAll('input[name="relFor"]')
                        .prop('checked', true);
                }
                if (rel.hateful) {
                    element
                        .nextAll('input[name="relHat"]')
                        .prop('checked', true);
                }
            ...
            default:
                ...
                element = $(this.element)
                    .find('.burner-gear select[name="itemName"]')
                    .filter((_, e) => !$(e).val())
                    .first();
                // Set the value, creating a new option if necessary
                if (
    ...
    _calculateRelationshipCost(target: JQuery<HTMLElement>): void {
        ...
        if (!parent.children("input[name='relationshipName']").val()) {
            parent
                .parent()
                .children("input[name='relationshipCost']")
                .val(0)
                .trigger('change');
            return;
        }
    ...
    private _calculateRepAffCost(target: JQuery): void {
        ...
        if (!parent.children("input[name='reputationName']").val()) {
            parent
                .children("input[name='reputationCost']")
                .val(0)
                .trigger('change');
            return;
        }
        const isAff = !parent
            .children("input[name='reputationType']")
            .prop('checked');
        const dice =
            parseInt(
                parent.children("input[name='reputationDice']").val() as string
            ) || 0;
        if (isAff) {
            const result =
                dice === 1 ? 10 : dice === 2 ? 25 : dice === 3 ? 50 : 0;
            parent
                .children("input[name='reputationCost']")
                .val(result)
                .trigger('change');
            ...
            const result =
                dice === 1 ? 7 : dice === 2 ? 25 : dice === 3 ? 45 : 0;
            parent
                .children("input[name='reputationCost']")
                .val(result)
                .trigger('change');
    ...
    private _calculateSkillExponent(target: JQuery, html: JQuery): void {
        ...
        const advances =
            parseInt(
                skillRow.children("*[name='skillAdvances']").val() as string
            ) || 0;
        let root1exp =
            parseInt(html.find(`*[name='${root1}Spent']`).val() as string) || 1;
        const root2exp =
            parseInt(html.find(`*[name='${root2}Spent']`).val() as string) ||
            root1exp;
        const root1Shade =
            parseInt(
                html.find(`*[name='${root1}ShadeSpent']`).val() as string
            ) || 0;
        let root2Shade = parseInt(
            html.find(`*[name='${root2}ShadeSpent']`).val() as string
        );
        if (isNaN(root2Shade)) {
        ...
        skillRow
            .children("*[name='skillShadeRefund']")
            .val(-Math.min(root1Shade, root2Shade));
        skillRow
            .children("*[name='skillShade']")
            .val(Math.min(root1Shade, root2Shade))
            .trigger('change');
    }

    private _calculateAllSkillExponents(
        ...
        html.find("select[name='skillRoot1'], select[name='skillRoot2']").each(
            (_, element) => {
    ...
    private _calculateSkillWorth(e: JQuery.ChangeEvent): void {
        ...
        const advances =
            parseInt(
                parent.children("*[name='skillAdvances']").val() as string
            ) || 0;
        const shade =
            parseInt(parent.children("*[name='skillShade']").val() as string) ||
            0;
        const refund =
            parseInt(
                parent.children("*[name='skillShadeRefund']").val() as string
            ) || 0;
        const open = parent.children("*[name='skillOpened']").prop('checked')
            ? 1
            : 0;
        const training = parent
            .children("*[name='skillTraining']")
            .prop('checked')
            ? open
            : 0;
        const magical =
            parent.children("*[name='skillMagic']").prop('checked') && !training
                ? open
                : 0;
        parent
            .children("*[name='skillPtsWorth']")
            .val(advances + shade + open + training + magical + refund)
            .trigger('change');
    }
    ...
    _tryLoadTrait(e: JQuery.TriggeredEvent): void {
        ...
        if (!traitName) {
            inputTarget
                .siblings('.load-status')
                .removeClass('none loading fail success')
                .addClass('none');
            inputTarget.siblings("*[name='traitId']").val('');
            inputTarget
                .siblings("*[name='traitCost']")
                .val('0')
                .trigger('change');
            return;
        }
        ...
        if (!trait) {
            ...
            inputTarget
                ...
                .addClass('success');
            inputTarget
                .siblings("*[name='traitType']")
                .val(trait.system.traittype)
                .trigger('change');
            inputTarget.siblings("*[name='traitId']").val(trait.id);
            const cost =
                !trait.system.pointCost || isNaN(trait.system.pointCost)
                    ? 1
                    : trait.system.pointCost;
            inputTarget
                .siblings("*[name='traitCost']")
                .val(cost)
                .trigger('change');
    ...
    private _tryLoadSkill(e: JQuery.TriggeredEvent): void {
        ...
        if (!skill) {
            ...
            inputTarget
                ...
                .addClass('success');
            inputTarget
                .siblings("*[name='skillRoot1']")
                .val(skill.system.root1)
                .trigger('change');
            inputTarget
                .siblings("*[name='skillRoot2']")
                .val(skill.system.root2)
                .trigger('change');
            inputTarget
                .siblings("*[name='skillTraining']")
                .prop('checked', skill.system.training);
            inputTarget
                .siblings("*[name='skillMagic']")
                .prop('checked', skill.system.magical);
    ...
    private _tryLoadProperty(e: JQuery.TriggeredEvent): void {
        ...
        if (!property) {
            ...
            inputTarget
                .siblings("*[name='propertyCost']")
                .val(property.system.pointCost || 0)
                .trigger('change');
        }
    }
    ...
    private _tryLoadGear(e: JQuery.TriggeredEvent): void {
        ...
        if (!gear) {
            ...
            inputTarget
                .siblings("*[name='itemCost']")
                .val((gear.system as HasPointCost).pointCost || 0)
                .trigger('change');
        }
    }
    ...
    private _storeSum(
        ...
        html.find(`input[name="${targetName}"]`)
            .val(this._calculateSum(html, ...sourceNames))
            .trigger('change');
    }
    ...
    private _calculateSum(html: JQuery, ...sourceNames: string[]): number {
        const selector = sourceNames
            .map((a) => `input[name='${a}'], select[name='${a}']`)
            .join(', ');
        let total = 0;
        html.find(selector).each((_i, elem) => {
            total += parseInt($(elem).val() as string) || 0;
        });
        return total;
    }

    private async _finishBurning(
        e: JQuery.ClickEvent,
        html: JQuery
    ): Promise<unknown> {
        e.preventDefault();
        return Dialog.confirm({
            ...
            content:
                'You are about to submit the character burner worksheet result. If this is being done to an already burned character it may do some damage. Continue?',
            yes: async () => {
                ...
                skillData.forEach((s) => {
                    if (s.data) {
                        s.data.learning = false;
                    }
                });

                this.close();

                return this._parent.createEmbeddedDocuments(
                    'Item',
                    [
                        ...skillData,
                        ...traitData,
