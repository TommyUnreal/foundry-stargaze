...
export class DifficultyDialog extends Application {
    ...
    activateSocketListeners(): void {
        ...
        game.socket.on(
            ...
            ({
                ...
                type: string;
                data: { extendedTest: boolean; actorGroups: ActorTestGroup[] };
            }) => {
    ...
    persistExtendedTestData(): void {
        const data = {
            extendedTest: this.extendedTest,
    ...
    addDeferredTest({
        ...
        if (!existingGroup) {
            ...
            existingGroup.advancements = existingGroup.advancements.concat(
                entries.filter(
                    (e) =>
                        !existingGroup.advancements.find(
                            (a) =>
                                a.title === e.title &&
    ...
    assignDeferredTest({
        ...
        path,
        title,
    }: AssignTestOptions): void {
        if (actor) {
            const difficulty: TestString =
                diff === 'R'
                    ? 'Routine'
                    : diff === 'C'
                    ? 'Challenging'
            ...
            const group = this.actorGroups.find(
                (ag) => ag.id === actor.id
            ) as ActorTestGroup;
            group.advancements = group.advancements.filter(
                (a) => a.title !== title
            );
