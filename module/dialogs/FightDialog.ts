...
export class FightDialog extends ExtendedTestDialog<FightDialogData> {
    ...
    getData(): FightDialogData {
        ...
        if (!this.data.actors.length) {
            this.data.actors = (game.actors?.filter((i: BWActor) =>
                this.data.data.participantIds.includes(i.id)
            ) || []) as BWActor[];
            this.data.actors.sort((a, b) => {
                return this.data.data.participantIds.indexOf(a.id) >
                    this.data.data.participantIds.indexOf(b.id)
                    ? 1
                    : -1;
            });
        }
        ...
        data.participants.forEach((p) => {
            const actor = this.data.actors.find(
                (a) => a.id === p.id
            ) as BWActor;
            if (!actor) {
                return;
            ...
            p.showActions =
                (data.gmView && !p.gmHidden) ||
                (!data.gmView &&
                    this.data.actors.find((a) => a.id === p.id)?.isOwner);
            p.chosenWeaponLabel =
                p.weapons.find((x) => x.id === p.weaponId)?.label ?? '';
        });
    ...
    activateListeners(html: JQuery): void {
        ...
        html.find('select[name="newParticipant"]').on(
            'change',
            (e: JQuery.ChangeEvent) => this._addNewParticipant(e.target)
        );
        html.find('*[data-action="removeFighter"]').on(
            'click',
            (e: JQuery.ClickEvent) => {
                e.preventDefault();
        ...
        html.find('*[data-action="toggleHidden"').on(
            'click',
            (e: JQuery.ClickEvent) => {
                e.preventDefault();
        ...
        html.find('.fighters-grid input, .fighters-grid select').on(
            'change',
            (e: JQuery.ChangeEvent) =>
                this.updateCollection(e, this.data.data.participants)
        );
        ['Speed', 'Power', 'Agility', 'Skill', 'Steel'].forEach(
            (attr: string) => {
                html.find(`button[data-action="roll${attr}"]`).on(
                    'click',
                    (e: JQuery.ClickEvent) => {
                        this._handleRoll(e, attr.toLowerCase() as FightAttr);
                    }
                );
            }
        );
        html.find(
            'div[data-action="openSheet"], img[data-action="openSheet"]'
        ).on('click', (e: JQuery.ClickEvent) => {
            const id =
                e.currentTarget.attributes.getNamedItem('data-actor-id')
                    .nodeValue || '';
    ...
    private _handleRoll(e: JQuery.ClickEvent, type: FightAttr) {
        ...
        const engagementBonus = parseInt(
            this.data.data.participants[index].engagementBonus.toString()
        );
        const positionPenalty = parseInt(
            this.data.data.participants[index].positionPenalty.toString()
        );
        if (type === 'skill') {
            ...
            if (!itemIdString) {
                return notifyError(
                    'No weapon selected',
                    'A weapon (or bare fists) must be selected to determine which skill to use for the roll.'
                );
            }
            let attackIndex: number | undefined;
            if (itemIdString.indexOf('_') !== -1) {
                attackIndex = parseInt(
                    itemIdString.substr(itemIdString.indexOf('_') + 1)
                );
                itemIdString = itemIdString.substr(
                    0,
                    itemIdString.indexOf('_')
                );
            }
            return handleFightRoll({
                actor,
                type,
                itemId: itemIdString,
                attackIndex,
    ...
    private _addNewParticipant(target: HTMLSelectElement): void {
        ...
        this.data.data.participants.push({
    ...
    private _syncActors() {
        game.socket.emit(constants.socketName, {
            type: `syncActors${this.data.topic}`,
        });
    }
...
function toParticipantData(
    actor: BWCharacter | Npc
): Partial<ParticipantEntry> {
