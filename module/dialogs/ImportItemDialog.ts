    ...
    activateListeners(html: JQuery): void {
        ...
        html.find('input.new-item-dialog-search').on('input', (e) => {
            ...
            html.find('.search-grid > .search-entry').each((_, item) => {
                if (
                    this.sources.indexOf(item.dataset.itemSource as string) !==
                        -1 &&
        ...
        html.find('select[name="select-compendiums"]')
            .select2({
                multiple: true,
                width: '97%',
            })
                ...
                html.find('.search-grid > .search-entry').each((_, item) => {
                    if (
                        this.sources.indexOf(
                            item.dataset.itemSource as string
                        ) !== -1 &&
                        (item.dataset.itemName || '')
                            .toLowerCase()
                            .indexOf(this.searchTerm.toLowerCase()) !== -1
        ...
        html.find('option')
            .each((_, o) => {
                if (
                    ...
                    this.sources.indexOf(o.value) !== -1
                ) {
...
export async function addNewItem(options: AddItemOptions): Promise<unknown> {
    if (!options.itemType && !options.itemTypes) {
        throw Error(
            'Must provide one or more item types when adding new items'
        );
    }
    if (options.itemType && !options.itemTypes) {
    ...
    const loadExistingCallback = async (
        _html: HTMLElement | JQuery<HTMLElement>
    ) => {
                ...
                buttons: {
                    add: {
                        ...
                        callback: async (dialogHtml: JQuery) => {
                            const newItems = dialogHtml
                                .find('input:checked')
                                .map((_, element: HTMLInputElement) => {
                                    const itemRoot: TypeMissing = items.find(
                                        (s: BWItem) => s.id === element.value
                                    ) as BWItem;
                                    Object.assign(
                                        itemRoot.system,
                                        options.forcedData
                            ...
                            actor.setFlag(
                                constants.systemName,
                                'compendia',
                                dialogHtml.find('select').val()
                            );
                            actor.createEmbeddedDocuments<BWItem>(
                                'Item',
                                newItems
                            );
                        },
    ...
    makeNewButtons.loadExisting = {
        label: game.i18n.format('BW.dialog.importExisting', {
            type: options.itemType || 'item',
        }),
