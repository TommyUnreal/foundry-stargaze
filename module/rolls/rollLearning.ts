...
export async function handleLearningRollEvent(
    rollOptions: LearningRollEventOptions
): Promise<unknown> {
...
export function handleLearningRoll({
    actor,
    skill,
    extraInfo,
    dataPreset,
    onRollCallback,
}: LearningRollOptions): unknown {
...
async function buildLearningDialog({
    skill,
    statName,
    actor,
    extraInfo,
    dataPreset,
    onRollCallback,
}: LearningRollDialogSettings): Promise<unknown> {
    ...
    if (dataPreset) {
        if (dataPreset.optionalDiceModifiers) {
            dataPreset.optionalDiceModifiers.concat(
                ...rollModifiers.filter((r) => r.optional && r.dice)
            );
        }
        if (dataPreset.optionalObModifiers) {
            dataPreset.optionalObModifiers.concat(
                ...rollModifiers.filter((r) => r.optional && r.obstacle)
            );
        }
    }
    ...
    const data: LearningDialogData = mergeDialogData<LearningDialogData>(
        {
            difficulty: 3,
            name: game.i18n.format('BW.roll.beginnersLuckTest', {
                name: skill.name,
            }),
            bonusDice: 0,
            arthaDice: 0,
...
async function learningRollCallback(
    ...
    if (rollData.splitPool) {
        splitPoolRoll = await getSplitPoolRoll(
            rollData.splitPool,
            skill.system.open,
            skill.system.shade
        );
        ...
        case 'Routine/Difficult':
            ...
            const dialog = new Dialog({
                ...
                content: `<p>${game.i18n.localize(
                    'BW.roll.learningAssignmentBody1'
                )}</p><p>${game.i18n.localize(
                    'BW.roll.learningAssignmentBody2'
                )}</p>`,
                buttons: {
                    skill: {
                    ...
                    stat: {
                        label: game.i18n.localize(
                            'BW.roll.learningApplyDifficult'
                        ),
                        callback: async () =>
                            advanceBaseStat(
                                skill,
