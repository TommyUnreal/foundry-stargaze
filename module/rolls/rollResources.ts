...
export async function handleResourcesRollEvent({
    sheet,
    dataPreset,
...
export async function handleResourcesRoll({
    actor,
    stat,
    dataPreset,
}: ResourcesRollOption): Promise<unknown> {
...
async function resourcesRollCallback(
    ...
    if (!isSuccess) {
        const taxAmount =
            rollData.difficultyGroup === 'Challenging'
                ? rollData.difficultyTotal - parseInt(roll.result)
                : rollData.difficultyGroup === 'Difficult'
                ? 2
                : 1;
        const taxMessage = new Dialog({
            title: game.i18n.localize('BW.dialog.failResources'),
            content: `<p>${game.i18n
                .localize('BW.dialog.failResourcesText1')
                .replace(
                    '{diff}',
                    rollData.difficultyGroup
                )}</p><p>${game.i18n.localize(
                'BW.dialog.failResourcesText2'
            )}</p><hr/>`,
            buttons: {
                full: {
                    label: `${game.i18n
                        .localize('BW.dialog.fullTax')
                        .replace('{amt}', taxAmount.toString())}`,
                    callback: () =>
                        actor.taxResources(taxAmount, rollData.fundDice),
                },
                cut: {
    ...
    await actor.addAttributeTest(
        stat,
        game.i18n.localize('BW.resources'),
        'resources',
        rollData.difficultyGroup,
        isSuccess
    );
